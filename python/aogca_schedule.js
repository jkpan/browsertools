...
function onOpen() {
  ...
  ui.createMenu('TPCAOG擴充').
    ...
    addItem('從輪值機器人還原(restore)', 'restoreSheet').
    addSeparator().
    addSubMenu(ui.createMenu('產生下季順序(restore)').
      addItem('領會', 'regenMemberlist0').
      addItem('領詩(主日)', 'regenMemberlist1').
      addItem('領詩(禱告會)', 'regenMemberlist2').
      addItem('助唱(1)', 'regenMemberlist3').
      addItem('助唱(2)', 'regenMemberlist4')).
    addToUi();
}

function regenMemberlist0() {
  regenMemberlist_('領會');
}

function regenMemberlist1() {
  regenMemberlist_('領詩', '主日');
}

function regenMemberlist2() {
  regenMemberlist_('領詩', '禱告會');
}

function regenMemberlist3() {
  regenMemberlist_('助唱(1)');
}

function regenMemberlist4() {
...
function regenMemberlist_(item, type) {
  ...
  for (let col = s_col; col <= col_max; col++) {
    let title = sheet.getRange(2, col).getValue();
  ...
  let title = '';
  let list = [];
  for (let row = row_max; row >= s_row; row--) {
    if (type && type.length > 0) {
      let comtype = sheet.getRange(row, 2).getValue().trim();
      if (comtype && !comtype.includes(type)) {
        continue;
      }
    }
    let name = sheet.getRange(row, s_col).getValue().trim();
    if (name.length == 0) continue;
    Logger.log(`(${row}, ${s_col}) ` + name);
    if (list.length == 0) {
      list[0] = name;
      continue;
    }
    for (let i = 0; i < list.length; i++) {
      if (name === list[i]) break; //Logger.log(" Equal break!");
      if (i == list.length - 1) {
        list.unshift(name);
        break;
      }
